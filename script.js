document.addEventListener("DOMContentLoaded", () => {
    ...
    botonInfoPersonal.addEventListener('click', () => {
        ...
        if (expanded) {
            ...
            botonInfoPersonal.focus(); // Opcional, regresar foco al botón al cerrar
        } else {
            fetch('info.xml')
                .then(response => {
                    if (!response.ok) {
                ...
                .then(data => {
                    ...
                    divInfoPersonal.innerHTML = `
                        <h3 tabindex="0">Información Personal</h3>
                        <ul>
                    ...
                    // Mover foco al contenido para que se pueda navegar con tab
                    divInfoPersonal.querySelector('h3').focus();
                })
                .catch(error => {
                    divInfoPersonal.innerHTML = `<p style="color: red;">Error cargando información: ${error.message}</p>`;
                    divInfoPersonal.removeAttribute('hidden');
                    botonInfoPersonal.setAttribute('aria-expanded', 'true');
                    divInfoPersonal.querySelector('p').focus();
                });
        }
    });
});
